...
public class RecentsTvActivity extends Activity implements OnPreDrawListener {
    ...
    private final static int MSG_DELETE_APP = 1;
    ...
    private TextView mDismiss_all_text;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //[Sen5-Jesseyao-2017-11-02] Set background as system wallpaper {
        WallpaperManager wallpaperManager = (WallpaperManager) getSystemService(Context.WALLPAPER_SERVICE);
        mRecentsView.setBackground(wallpaperManager.getDrawable());
        //[Sen5-Jesseyao-2017-11-02] Set background as system wallpaper }
        ...
        mDismiss_all_text = (TextView)findViewById(R.id.dismiss_all_text);
    }
    ...
    public void onResume() {
        ...
        //Delete all apps, add by wujiang 2018-02-26 start. 
        mDismiss_all_text.setVisibility(View.INVISIBLE);
        mDismissAlpha = getResources().getFloat(R.integer.dismiss_unselected_alpha);
        final Button dismiss_all_button = (Button)findViewById(R.id.dismiss_all_button);
        dismiss_all_button.setAlpha(mDismissAlpha * 1.0f);
        if(mTaskStackHorizontalGridView.getStack().getTaskCount() < 1) {
        	dismiss_all_button.setVisibility(View.GONE);
        }
        dismiss_all_button.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				mDismissAppHandler.sendEmptyMessage(MSG_DELETE_APP);
			}
		});
        
        dismiss_all_button.setOnFocusChangeListener(new OnFocusChangeListener() {
			
			@Override
			public void onFocusChange(View v, boolean hasFocus) {
				// TODO Auto-generated method stub
				if(hasFocus == true) {
					mDismiss_all_text.setVisibility(View.VISIBLE);
					dismiss_all_button.setAlpha(1.0f);
					...
					dismiss_all_button.setAnimation(animation);
					animation.startNow();
				} else {
					mDismiss_all_text.setVisibility(View.INVISIBLE);
					dismiss_all_button.setAlpha(mDismissAlpha * 1.0f);
        ...
        // Delete all apps, add by wujiang 2018-02-26 end.
	...
	//Add by wujiang 2018-02-26 start.
	private Animator.AnimatorListener getRemoveAtListener(final int position, final Task task) {
		return new Animator.AnimatorListener() {
			...
			public void onAnimationEnd(Animator animation) {
				mTaskStackViewAdapter.removeTask(task);
	...
	public Handler mDismissAppHandler = new Handler(Looper.getMainLooper()) {
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
				case MSG_DELETE_APP:
					//Log.d("wujiang", "receive MSG_DELETE_APP");
					int count = mTaskStackHorizontalGridView.getChildCount();
					if(count > 0) {
						TaskCardView cardview = (TaskCardView)mTaskStackHorizontalGridView.getChildAt(0);

						if(cardview instanceof TaskCardView) {
							cardview.startDismissTaskAnimation(getRemoveAtListener(0, cardview.getTask()));
						}
						mDismissAppHandler.sendEmptyMessageDelayed(MSG_DELETE_APP, 100);
					}
					break;
