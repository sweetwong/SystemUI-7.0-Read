...
public class ViewFocusAnimator implements View.OnFocusChangeListener {
    ...
    private void setFocusProgress(float level) {
		...
		if (mTargetView.isInDismissState()) {
			// 如果Dismiss all在Dismiss的下面，焦点从dismiss all 重回Dismiss状态后，圆X图标也要重回高亮状态。
			// add by wujiang 2018-02-26
			mTargetView.getDismissIconView().setAlpha(level);
		} else {
